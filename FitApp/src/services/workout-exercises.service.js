...
export const addExerciseToDay = async (dayId, userId, {exerciseId, sets, reps, rest}) => {
    ...
    return workoutExerciseRepo.createWorkoutExercise({
        id: uuidv4(), dayId, exerciseId, sets, reps, rest,
    });
}

export const getExercisesForDay = async (dayId, userId) => {
    const day = await dayRepo.findByIdAndUserId(dayId, userId);
    if (!day) {
        throw new Error("Day not found");
    }
    return workoutExerciseRepo.findAllByDayId(dayId);
}

export const updateWorkoutExercise = async (id, userId, {sets, reps, rest}) => {
